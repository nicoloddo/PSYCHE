...
DEFAULT_transcriptions_dir_relative = 'transcriptions_and_alignments/'
...
def setup_global_paths(dataset_dir_relative = DEFAULT_dataset_dir_relative, transcriptions_dir_relative = DEFAULT_transcriptions_dir_relative, wavs_dir_relative = DEFAULT_wavs_dir_relative, temp_wavs_dir_relative = DEFAULT_temp_wavs_dir_relative, breath_labels_dir_relative = DEFAULT_breath_labels_dir_relative):
    ...
    return DATASET_DIR, TRANSCR_DIR, WAVS_DIR, TEMP_WAVS_DIR, BREATH_DF, WAV_FILENAMES


DATASET_DIR, TRANSCR_DIR, WAVS_DIR, TEMP_WAVS_DIR, BREATH_DF, WAV_FILENAMES = setup_global_paths() # Here it uses the default ones


#  AUDIO EXPLORATION FUNCTIONS

def get_file_json(filename, json_dir, suffix = ''): # with wav filename
    with open(TRANSCR_DIR + json_dir + filename[:-4] + suffix + '.json', 'r') as f:
        data = json.load(f)
    return dict(data)
