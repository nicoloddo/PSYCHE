...
transcriptions_dir_relative = 'transcriptions_and_alignments/'
...
def gentle_align(transcriptions_dir_name, aligned_dir_name, sorted_dir_name, port = '32768', except_filenames = [], transcr_suffix = ''):
    ...
    for filename in WAV_FILENAMES:
        ...
        data = x.json()
        with open(TRANSCR_DIR + aligned_dir_name + 'json/' + filename[:-4] + '.json', 'w') as f:
            json.dump(data, f, indent = 1)
        ...
        with open(TRANSCR_DIR + aligned_dir_name + filename[:-4] + '.txt', 'w') as f:
            f.write(transcript)
        ...
        with open(TRANSCR_DIR + sorted_dir_name + filename[:-4] + '.txt', 'w') as f:
            f.write(transcript)
        
        sorted_dict = {'words':sorted_align}
        with open(TRANSCR_DIR + sorted_dir_name + 'json/' + filename[:-4] + '.json', 'w') as f:
...
def gentle_mfa_align(gentle_align_dir, mfa_align_dir, gentle_mfa_align_dir, except_filenames = ['devel_10.wav', 'test_08.wav', 'train_01.wav', 'train_10.wav', 'train_14.wav']): 
    ...
    for filename in WAV_FILENAMES:
        ...
        with open(TRANSCR_DIR + gentle_align_dir + filename[:-4] + '.json', 'r') as f:
            gentle_data = json.load(f)
            gentle_dict = dict(gentle_data)
            
        with open(TRANSCR_DIR + mfa_align_dir + filename[:-4] + '.json', 'r') as f:
        ...
        with open(TRANSCR_DIR + gentle_mfa_align_dir + 'json/' + filename[:-4] + '.json', 'w') as f:
            json.dump(sorted_dict, f, indent = 1)